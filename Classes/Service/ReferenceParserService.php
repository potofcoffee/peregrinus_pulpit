...
class ReferenceParserService
{
    ...
    /** @var ReferenceParserService */
    protected static $instance = null;

    /**
     * @return ReferenceParserService
     */
    public static function getInstance(): ReferenceParserService
    {
    ...
    {
        $this->map = yaml_parse_file(PEREGRINUS_PULPIT_BASE_PATH.'Assets/Bible/BookMap.yaml');
        $this->buildRawMap();
    }
...
//if (substr($reference, 0, 3)=='Heb') Debugger::dumpAndDie($reference);
        ...
        $reference = strtr(trim($originalReference), [
            '. ' => '.',
            ' -' => '-',
            '- ' => '-',
            ') ' => '.',
    ...
    protected function parseSingleReference(string $reference, array $defaults): array
    {
        ...
        if (trim($verseNumeric)=='') $verseNumeric=$defaults['verse'];
        return [
            'book' => $book,
